...
# 文字起こし
def transcribe_gcs(gcs_uri, file_path, file_path2):
...
# flacファイルの生成
def create_flac(directory, filename):
    original_file = directory + filename + '.mp3'
    convereted_file = directory + filename + '.flac'
    cmd =  'ffmpeg -i "'+ original_file + '" -ac 1 "' + convereted_file + '"'
...
print(filename)
#filename = 'resultshort' # 変数．ファイル名をここだけ変更する


### 変数設定
directory = '/Volumes/GoogleDrive/マイドライブ/Research/SpeechtoText/'
bucket_name = "research_design"
# uri = 'gs://research_design/' + filename + '.flac'
uri = 'gs://'+ bucket_name + '/' + filename + '.flac'
file_path1 = directory + filename + 'word.csv' #生の読み出し結果
file_path2 = directory + filename + '.txt' #生の読み出し結果txt
...
### 実行コマンド郡
create_flac(directory, filename)
print("endcreateflac")
upload_blob(bucket_name, source_file_name, destination_blob_name)
print ("enduploadblob")
transcribe_gcs(uri, file_path1, file_path2)
print("endtranscribe")
